...
public class ArbolBinarioBusqueda<T> {
    ...
    private boolean Borrar(NodoABB<T> R, T Infor) {
        ...
        while (R != null) {
            if (Llave.compareTo(R.Info.toString()) < 0) {
                ...
                R = R.getSubIzq();
                b = false;
            } else if (Llave.compareTo(R.Info.toString()) > 0) {
                Ant = R;
                R = R.getSubDer();
                b = true;
            } else {
        ...
        {
            // Buscamos el nodo mas peque�o del sub�rbol derecho
            NodoABB<T> Temp = R.getSubDer();
            NodoABB<T> Aux = R;
    ...
    private boolean Busca(NodoABB<T> Raiz, T Dato) {
        if (Raiz == null) {
            return false;
        }
        if (Dato.toString().compareTo(Raiz.Info.toString()) == 0) { // Lo encontró
            Dr = Raiz.Info;
            return true;
        }
        if (Dato.toString().compareTo(Raiz.Info.toString()) < 0) {
            return Busca(Raiz.getSubIzq(), Dato);
        } else {
            return Busca(Raiz.getSubDer(), Dato);
        }
    }
}
